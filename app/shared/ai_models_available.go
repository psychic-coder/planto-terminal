...
When checking for sufficient credits on Planto Cloud, we use MaxOutputTokens-InputTokens, since this is the maximum that could hypothetically be used.
...
'ModelId' is the identifier for the model on the Planto side—it must be unique per provider. We have this so that models with the same name and provider, but different settings can be differentiated.
