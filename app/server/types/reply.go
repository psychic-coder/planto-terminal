...
func (r *ReplyParser) AddChunk(chunk string, addToTotal bool) {
	...
	if r.maybeFilePath != "" && !r.isInMoveBlock && !r.isInRemoveBlock && !r.isInResetBlock {
		...
		if strings.HasPrefix(prevFullLineTrimmed, "<PlantoBlock") {
			if verboseLogging {
	...
	if r.currentFilePath == "" && !r.isInMoveBlock && !r.isInRemoveBlock && !r.isInResetBlock {
		...
		if LineHasXmlPath(prevFullLineTrimmed) {
			if verboseLogging {
				log.Println("Line has XML-style PlantoBlock tag")
			}
		...
		if prevFullLineTrimmed == "</PlantoBlock>" {
			if verboseLogging {
		...
		if prevFullLineTrimmed == "<EndPlantoFileOps/>" {
			if verboseLogging {
...
func LineHasXmlPath(line string) bool {
	return strings.HasPrefix(line, "<PlantoBlock") && strings.Contains(line, `path="`)
}
...
func extractFilePath(line string) string {
	// Handle XML-style PlantoBlock tag
	if strings.HasPrefix(line, "<PlantoBlock") {
