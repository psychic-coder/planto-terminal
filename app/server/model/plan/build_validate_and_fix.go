...
import (
	...
	"log"
	"math/rand"
	diff_pkg "planto-server/diff"
	"planto-server/model"
	"planto-server/model/prompts"
...
func (fileState *activeBuildStreamFileState) buildValidate(
	...
	stop := []string{"<PlantoFinish/>"}
	if params.validateOnly {
		stop = []string{"<PlantoComments>", "<PlantoReplacements>"}
	}
...
func handleXMLResponse(
	...
	if strings.Contains(content, "<PlantoCorrect/>") {
	...
	replacementsOuter := utils.GetXMLContent(content, "PlantoReplacements")
	...
	if strings.Contains(content, "<PlantoIncorrect/>") {
		split := strings.Split(content, "<PlantoIncorrect/>")
		problem = split[0]
	} else if strings.Contains(content, "<PlantoReplacements>") {
		split := strings.Split(content, "<PlantoReplacements>")
		problem = split[0]
	}
