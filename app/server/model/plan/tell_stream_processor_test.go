...
func TestBufferOrStream(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			initialState: &chunkProcessor{
				fileOpen:                true,
				contentBuffer:           `<PlantoBlock lang="go" path="main.go">` + "\n",
				awaitingBlockOpeningTag: true,
			},
			chunk:           `package`,
			maybeFilePath:   "",
		...
		},
		{
			...
			chunk:           `<PlantoBlock lang="go" path="main.go">` + "\npackage",
			maybeFilePath:   "",
			currentFilePath: "main.go",
			want: bufferOrStreamResult{
				shouldStream: true,
		...
		},
		{
			...
			wantState: &chunkProcessor{
				awaitingBlockOpeningTag: true,
				contentBuffer:           "something<PlantoBlock lang=\"go\" path=\"main",
			},
		},
		{
...

		{
			...
			chunk:        "hello <PlantoFinish/>bye",
			manualStop:   []string{"<PlantoFinish/>"},
			want: bufferOrStreamResult{
		...
		},
		{
			...
			chunk:      "<PlantoFin", // no '<' in second part
			manualStop: []string{"<PlantoFinish/>"},
			want: bufferOrStreamResult{
			...
			wantState: &chunkProcessor{
				contentBuffer: "<PlantoFin", // prefix is buffered
			},
		},
		{
			...
			initialState: &chunkProcessor{
				contentBuffer: "<PlantoFin", // leftover from previous call
			},
