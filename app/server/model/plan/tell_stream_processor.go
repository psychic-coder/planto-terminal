...
var openingTagRegex = regexp.MustCompile(`<PlantoBlock\s+lang="(.+?)"\s+path="(.+?)".*?>`)
...
func (state *activeTellStreamState) processChunk(choice types.ExtendedChatCompletionStreamChoice) processChunkResult {
	...
	if processor.fileOpen && strings.HasSuffix(active.CurrentReplyContent+content, "</PlantoBlock>") {
		if verboseLogging {
...
func (processor *chunkProcessor) bufferOrStream(content string, parserRes *types.ReplyParserRes, currentStage shared.CurrentStage, manualStopSequences []string) bufferOrStreamResult {
	...
	if awaitingTag {
		...
		if processor.awaitingBlockOpeningTag {
			...
			if parserRes.CurrentFilePath != "" {
				...
				if matched {
					...
					if verboseLogging {
						log.Printf("Opening <PlantoBlock> tag is missing even though parserRes.CurrentFile is set - something is wrong: %s\n", content)
					}
					processor.awaitingBlockOpeningTag = false
					processor.fileOpen = false
				...
				if len(split) > 1 {
					...
					if strings.HasPrefix(`PlantoBlock lang="`, last) {
						if verboseLogging {
							log.Println("strings.HasPrefix(`PlantoBlock lang=", last)
						}
						shouldStream = false
						matchedPrefix = true
					} else if strings.HasPrefix(last, `PlantoBlock lang="`) {
						if verboseLogging {
			...
			if parserRes.CurrentFilePath == "" {
				if strings.Contains(content, "</PlantoBlock>") {
					shouldStream = true
					processor.awaitingBlockClosingTag = false
					processor.fileOpen = false
					// replace </PlantoBlock> with ``` to close the markdown code block
					content = strings.ReplaceAll(content, "</PlantoBlock>", "```")
				} else {
					log.Printf("Closing </PlantoBlock> tag is missing even though parserRes.CurrentOperation is nil - something is wrong: %s\n", content)
					processor.awaitingBlockClosingTag = false
					shouldStream = true
				}
			}
			...
			if strings.Contains(content, "<EndPlantoFileOps/>") {
				if verboseLogging {
					log.Printf("Found <EndPlantoFileOps/>\n")
				}
				processor.awaitingOpClosingTag = false
				content = strings.Replace(content, "\n<EndPlantoFileOps/>", "", 1)
				content = strings.Replace(content, "<EndPlantoFileOps/>", "", 1)
				shouldStream = true
			}
		}
		...
		if parserRes.MaybeFilePath != "" && parserRes.CurrentFilePath == "" {
			...
			// this will set processor.awaitingBlockOpeningTag to true if the content starts with any prefix of<PlantoBlock lang=" *or* any prefix of a full opening tag
		...
		if parserRes.CurrentFilePath != "" {
			...
			if strings.Contains(content, "</PlantoBlock>") {
				if verboseLogging {
					log.Println("strings.Contains(content, \"</PlantoBlock>\")")
				}
				processor.awaitingBlockClosingTag = true
			} else {
				if verboseLogging {
					log.Println("not strings.Contains(content, \"</PlantoBlock>\")")
				}
				split := strings.Split(content, "<")
				// log.Printf("split: %v\n", split)
				if len(split) > 1 {
					...
					last := split[len(split)-1]
					// log.Printf("last: %s\n", last)
					if strings.HasPrefix("/PlantoBlock>", last) {
						if verboseLogging {
							log.Println("strings.HasPrefix(\"/PlantoBlock>\", last)")
						}
						processor.awaitingBlockClosingTag = true
					}
				}
			...
			if strings.Contains(content, "<EndPlantoFileOps/>") {
				if verboseLogging {
					log.Println("strings.Contains(content, \"<EndPlantoFileOps/>\")")
				}
				processor.awaitingOpClosingTag = true
			} else {
				if verboseLogging {
					log.Println("not strings.Contains(content, \"<EndPlantoFileOps/>\")")
				}
				split := strings.Split(content, "<")
				if len(split) > 1 {
					if verboseLogging {
					...
					last := split[len(split)-1]
					if strings.HasPrefix("EndPlantoFileOps/>", last) {
						if verboseLogging {
							log.Println("strings.HasPrefix(\"EndPlantoFileOps/>\", last)")
						}
						processor.awaitingOpClosingTag = true
					}
				}
			}
		} else if strings.Contains(content, "</PlantoBlock>") {
			if verboseLogging {
				log.Println("strings.Contains(content, \"</PlantoBlock>\")")
			}
			content = strings.Replace(content, "</PlantoBlock>", "```", 1)
		} else if strings.Contains(content, "<EndPlantoFileOps/>") {
			if verboseLogging {
				log.Println("strings.Contains(content, \"<EndPlantoFileOps/>\")")
			}
			content = strings.Replace(content, "\n<EndPlantoFileOps/>", "", 1)
...
func replaceCodeBlockOpeningTag(content string, replaceWithFn func(lang string) string) (bool, string) {
	// check for opening tag matching <PlantoBlock lang="..." path="...">
	...
	if match != nil {
		...
		return true, strings.Replace(content, "<PlantoBlock>", replaceWithFn(""), 1)
	}
