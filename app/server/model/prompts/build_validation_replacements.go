...
If the changes were applied *correctly*, you MUST output a <PlantoCorrect/> tag, followed by a <PlantoFinish/> tag, then end your response, like this:
...
Next, you MUST output a <PlantoIncorrect/> tag, and then proceed to output the <PlantoComments/> tag and the <PlantoReplacements/> tag with at least one <Replacement> element (see below for details). Example:
...
Next, if the changes were applied *incorrectly*, you must analyze the *original file* and the *proposed updates* and output a <PlantoReplacements> element that applies the changes described in the *proposed updates* to the *original file* in order to produce a final, valid resulting file with all changes correctly applied.

CRITICALLY IMPORTANT: When applying changes with replacements, NO REFERENCE COMMENTS CAN BE PRESENT IN THE RESULTING FILE. All reference comments (as listed in the <PlantoComments> element above) ABSOLUTELY MUST be replaced with the code they refer to in the *original file*.

Now output a <PlantoReplacements> element that contains all the replacements needed to correctly apply the changes described in the *proposed updates* to the *original file*. The <PlantoReplacements> element MUST contain at least one <Replacement> element.
...
IMPORTANT RULES:
1. If your evaluation finds ANY issues, you MUST use <PlantoIncorrect/> followed by a <PlantoComments> element and a <PlantoReplacements> element with at least one <Replacement> element.
2. If your evaluation finds NO issues, you MUST use <PlantoCorrect/> then a <PlantoFinish/> element. Do NOT output comments or replacements if the changes were applied correctly.
...
5. Always include reasoning in a '## Evaluate Diff' section prior to outputting the <PlantoCorrect/> or <PlantoIncorrect/> tags.
