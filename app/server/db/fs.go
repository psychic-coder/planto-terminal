...
func init() {
	...
	log.Println("Planto server home dir:", home)
	log.Println("os.Getenv(PLANTO_BASE_DIR):", os.Getenv("PLANTO_BASE_DIR"))
	log.Println("GOENV:", os.Getenv("GOENV"))
	if os.Getenv("GOENV") == "development" && os.Getenv("LOCAL_MODE") == "1" {
		log.Println("Local mode enabled")
	}

	BaseDir = os.Getenv("PLANTO_BASE_DIR")

	if BaseDir == "" {
		if os.Getenv("GOENV") == "development" {
			BaseDir = filepath.Join(home, "planto-server")
		} else {
			BaseDir = "/planto-server"
		}
	}
