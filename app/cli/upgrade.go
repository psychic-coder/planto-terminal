...
func checkForUpgrade() {
	if os.Getenv("PLANTO_SKIP_UPGRADE") != "" {
		return
	}
	...
	defer cancel()
	latestVersionURL := "https://planto.ai/v2/cli-version.txt"
	...
	if latestVersion.GreaterThan(currentVersion) {
		term.StopSpinner()
		fmt.Println("A new version of Planto is available:", color.New(color.Bold, term.ColorHiGreen).Sprint(versionStr))
		...
		if confirmed {
			...
			fmt.Println("Note: set PLANTO_SKIP_UPGRADE=1 to stop upgrade prompts")
		}
	}
}
...
func doUpgrade(version string) error {
	...
	downloadURL := fmt.Sprintf("https://github.com/plandex-ai/planto/releases/download/%s/planto_%s_%s_%s.tar.gz", escapedTag, version, runtime.GOOS, runtime.GOARCH)
	...
	for {
		...
		if header.Typeflag == tar.TypeReg && (header.Name == "planto" || header.Name == "planto.exe") {
