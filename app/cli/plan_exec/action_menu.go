...
func handleHotkeyOption(option hotkeyOption, diffs []string, params ExecParams) {
	...
	if option.char == "d" {
		fmt.Println()
		_, err := lib.ExecPlantoCommandWithParams([]string{"diffs", "--ui", "--from-tell-menu"}, lib.ExecPlantoCommandParams{
		...
		fmt.Println()
		_, err := lib.ExecPlantoCommandWithParams([]string{"diffs", "--git"}, lib.ExecPlantoCommandParams{
			DisableSuggestions: true,
		})
		if err != nil {
			fmt.Printf("\nError showing diffs: %v\n", err)
		...
		fmt.Print("(a)")
		fmt.Println()
		_, err := lib.ExecPlantoCommand([]string{"apply"})
		if err != nil {
		...
		fmt.Println()
		_, err := lib.ExecPlantoCommand([]string{"reject"})
		if err != nil {
		...
		_, err := lib.ExecPlantoCommand([]string{"apply", "--full"})
		if err != nil {
			fmt.Printf("\nError applying changes: %v\n", err)
		}
		fmt.Println()
