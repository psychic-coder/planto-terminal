...
func MigrateLegacyProjectFile(currentUserId string) {
	...
	// New formats map to a user id so we can handle multiple accounts in the same planto dir
	projectPath := filepath.Join(fs.PlantoDir, "project.json")
	if _, err := os.Stat(projectPath); err == nil {
		...
		v2Path := filepath.Join(fs.PlantoDir, "projects-v2.json")
...
func MigrateLegacyCurrentPlanFile(currentUserId string) {
	...
	planPath := filepath.Join(fs.HomePlantoDir, CurrentProjectId, "current_plan.json")
	...
	if _, err := os.Stat(planPath); err == nil {
		...
		v2Path := filepath.Join(fs.HomePlantoDir, CurrentProjectId, "current-plans-v2.json")
...
func MigrateLegacyPlanSettingsFile(currentUserId string) {
	...
	settingsPath := filepath.Join(fs.HomePlantoDir, CurrentProjectId, CurrentPlanId, "settings.json")
	if _, err := os.Stat(settingsPath); err == nil {
		...
		v2Path := filepath.Join(fs.HomePlantoDir, CurrentProjectId, CurrentPlanId, "settings-v2.json")
