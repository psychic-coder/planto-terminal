...
func init() {
	ReplSettingsDir = filepath.Join(fs.HomePlantoDir, "repl_settings")
}
...
// ExecPlantoCommand spawns the same binary, wiring std streams directly so you
// don't have to capture output. Any os.Exit calls in the child won't kill your REPL.
func ExecPlantoCommand(args []string) (string, error) {
	return ExecPlantoCommandWithParams(args, ExecPlantoCommandParams{})
}

type ExecPlantoCommandParams struct {
	DisableSuggestions bool
	SessionId          string
}

func ExecPlantoCommandWithParams(args []string, params ExecPlantoCommandParams) (string, error) {
	// Create temp file
	tmpFile, err := os.CreateTemp("", "planto-output-*")
	if err != nil {
		return "", err
	}
	...
	if os.Getenv("PLANTO_REPL") == "" {
		...
		env = append(env,
			"PLANTO_REPL=1",
			"PLANTO_REPL_OUTPUT_FILE="+tmpPath,
			"PLANTO_COLUMNS="+strconv.Itoa(columns),
			"PLANTO_STREAM_FOREGROUND_COLOR="+streamForegroundColor.Sequence(false),
			"GLAMOUR_STYLE="+glamourStyle,
			"PLANTO_SKIP_UPGRADE=1",
		)

		if params.SessionId != "" {
			env = append(env, "PLANTO_REPL_SESSION_ID="+params.SessionId)
		}
	}

	if params.DisableSuggestions {
		env = append(env, "PLANTO_DISABLE_SUGGESTIONS=1")
	}
