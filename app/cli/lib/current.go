...
func resolveProject(mustResolve, shouldCreate bool) {
	if fs.PlantoDir == "" {
		var err error
		if shouldCreate {
			_, _, err = fs.FindOrCreatePlanto()
		} else {
			fs.FindPlantoDir()
		}

		if err != nil && mustResolve {
			term.OutputErrorAndExit("error finding or creating planto: %v", err)
		}
	}

	if (fs.PlantoDir == "" || fs.ProjectRoot == "") && mustResolve {
		fmt.Printf(
			"🤷‍♂️ No plans in current directory\nTry %s to create a plan or %s to see plans in nearby directories\n",
			color.New(color.Bold, term.ColorHiCyan).Sprint("planto new"),
			color.New(color.Bold, term.ColorHiCyan).Sprint("planto plans"))
		os.Exit(0)
	}
	...
	// check if projects-v2.json exists in PlantoDir
	path := filepath.Join(fs.PlantoDir, "projects-v2.json")
	_, err := os.Stat(path)
	...
	HomeCurrentProjectDir = filepath.Join(fs.HomePlantoDir, CurrentProjectId)
...
func mustInitProject(existingSettings *types.CurrentProjectSettingsByAccount) {
	...
	// write current-plans-v2.json to PlantoHomeDir/[projectId]/current-plans-v2.json
	dir := filepath.Join(fs.HomePlantoDir, CurrentProjectId)
