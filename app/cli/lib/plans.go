...
func WriteCurrentPlan(id string) error {
	if fs.HomePlantoDir == "" {
		return fmt.Errorf("HomePlantoDir not set")
	}
...
func WriteCurrentBranch(branch string) error {
	...
	dir := filepath.Join(fs.HomePlantoDir, CurrentProjectId, CurrentPlanId)
...
func GetCurrentBranchNamesByPlanId(planIds []string) (map[string]string, error) {
	if fs.HomePlantoDir == "" {
		return nil, fmt.Errorf("HomePlantoDir not set")
	}
...
func getPlanCurrentBranch(planId string) (string, error) {
	...
	v2Path := filepath.Join(fs.HomePlantoDir, CurrentProjectId, planId, "settings-v2.json")
