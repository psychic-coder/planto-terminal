...
var replCmd = &cobra.Command{
	Use:   "repl",
	Short: "Start interactive Planto REPL",
	Run:   runRepl,
}
...
func runRepl(cmd *cobra.Command, args []string) {
	...
	if lib.CurrentPlanId == "" {
		os.Setenv("PLANTO_DISABLE_SUGGESTIONS", "1")
		args := []string{}
		...
		os.Setenv("PLANTO_DISABLE_SUGGESTIONS", "")
		afterNew = true
	}
	...
	p = prompt.New(
		...
		prompt.WithTitle("Planto "+version.Version),
...
func executor(in string, p *prompt.Prompt) {
	...
	// Handle planto/pto command prefix
	if strings.HasPrefix(lastLine, "planto ") || strings.HasPrefix(lastLine, "pto ") {
		fmt.Println()
		parts := strings.Fields(lastLine)
		if len(parts) > 1 {
			args := parts[1:] // Skip the "planto" or "pto" command
			_, err := lib.ExecPlantoCommandWithParams(args, lib.ExecPlantoCommandParams{
				SessionId: sessionId,
			})
			if err != nil {
	...
	if lib.CurrentReplState.Mode == lib.ReplModeTell {
		...
		output, err := lib.ExecPlantoCommandWithParams(args, lib.ExecPlantoCommandParams{
			SessionId: sessionId,
		})
		if err != nil {
...
func replWelcome(params replWelcomeParams) {
	...
	color.New(color.FgHiWhite, color.BgBlue, color.Bold).Print(" 👋 Welcome to Planto ")
...
func handleRunCommand(args []string) error {
	...
	// Execute the command
	_, err := lib.ExecPlantoCommand(cmdArgs)
	if err != nil {
...
func execWithInput(params execWithInputParams) execWithInputResult {
	...
	default:
		...
		if matchedCmd != "" {
			...
			_, err := lib.ExecPlantoCommandWithParams(execArgs, lib.ExecPlantoCommandParams{
				SessionId: sessionId,
			})
			if err != nil {
				color.New(term.ColorHiRed).Printf("Error executing command: %v\n", err)
