...
var CliCommands = []CmdConfig{
	{"", "", "start the Planto REPL", false},

	// {"--full", "", fmt.Sprintf("start the Planto REPL with auto-mode %s", "'full'"), false},
	// {"--semi", "", fmt.Sprintf("start the Planto REPL with auto-mode %s", "'semi'"), false},
	// {"--plus", "", fmt.Sprintf("start the Planto REPL with auto-mode %s", "'plus'"), false},
	// {"--basic", "", fmt.Sprintf("start the Planto REPL with auto-mode %s", "'basic'"), false},
	// {"--none", "", fmt.Sprintf("start the Planto REPL with auto-mode %s", "'none'"), false},

	// {"--daily", "", fmt.Sprintf("start the Planto REPL with %s model pack", "'daily-driver'"), false},
	// {"--strong", "", fmt.Sprintf("start the Planto REPL with %s model pack", "'strong'"), false},
	// {"--cheap", "", fmt.Sprintf("start the Planto REPL with %s model pack", "'cheap'"), false},
	// {"--oss", "", fmt.Sprintf("start the Planto REPL with %s model pack", "'oss'"), false},
	...
	{"usage", "", "show Planto Cloud current balance and usage report", true},
	{"usage --today", "", "show Planto Cloud usage for the day so far", true},
	{"usage --month", "", "show Planto Cloud usage for the current billing month", true},
	{"usage --plan", "", "show Planto Cloud usage for the current plan", true},

	{"usage --log", "", "show Planto Cloud transaction log", true},

	{"billing", "", "show Planto Cloud billing settings", true},
}
...
func printCmds(w io.Writer, prefix string, colors []color.Attribute, cmds ...string) {
	if os.Getenv("PLANTO_DISABLE_SUGGESTIONS") != "" {
		return
	}
	...
	for _, cmd := range cmds {
		...
		if IsRepl {
			...
			styled = color.New(colors...).Sprintf(" planto ")
		} else {
			styled = color.New(colors...).Sprintf(" planto %s ", cmd)
		}
...
func PrintCustomCmd(prefix, cmd, alias, desc string) {
	cmd = strings.Replace(cmd, alias, fmt.Sprintf("(%s)", alias), 1)
	// desc += color.New(color.FgWhite).Sprintf(" • alias → %s", fmt.Sprint(a'lias'))
	styled := color.New(color.Bold, color.FgHiWhite, color.BgCyan, color.FgHiWhite).Sprintf(" planto %s ", cmd)
	fmt.Printf("%s%s 👉 %s\n", prefix, styled, desc)
}

// PrintCustomHelp prints the custom help output for the Planto CLI
func PrintCustomHelp(all bool) {
	builder := &strings.Builder{}

	color.New(color.Bold, color.BgGreen, color.FgHiWhite).Fprintln(builder, " Usage ")
	color.New(color.Bold).Fprintln(builder, "  planto [command] [flags]")
	...
	color.New(color.Bold).Fprintln(builder, "  planto help # show basic usage")
	color.New(color.Bold).Fprintln(builder, "  planto help --all # show all commands")
	color.New(color.Bold).Fprintln(builder, "  planto [command] --help")
	fmt.Fprintln(builder)

	color.New(color.Bold, color.BgMagenta, color.FgHiWhite).Fprintln(builder, " Getting Started ")
	fmt.Fprintln(builder)
	fmt.Fprintf(builder, " 🚀 Start the Planto REPL in a project directory with %s or %s\n", color.New(color.Bold, color.BgCyan, color.FgHiWhite).Sprint(" planto "), color.New(color.Bold, color.BgCyan, color.FgHiWhite).Sprint(" pdx "))
	fmt.Fprintln(builder)
	fmt.Fprintf(builder, " 💻 You can also use any command outside the REPL with %s or %s\n", color.New(color.Bold, color.BgCyan, color.FgHiWhite).Sprint(" planto [command] "), color.New(color.Bold, color.BgCyan, color.FgHiWhite).Sprint(" pdx [command] "))
	fmt.Fprintln(builder)
	...
	if all {
		...
		color.New(color.Bold, color.BgHiBlue, color.FgHiWhite).Fprintln(builder, " Use 'planto help --all' or 'planto help -a' for a list of all commands ")
		fmt.Fprintln(builder)
